...
from common import get, now, write_attributes, write_brands, write_products
...
def crawl():
    ...
    page = 1
    while True:
        ps = get(f"{COLLECTION}?limit=250&page={page}").json().get("products", [])
        if not ps:
            break
        for p in ps:
...
def main():
    products = crawl()
    print(
        f"crawled {len(products)} Bluemercury makeup products, "
    ...
    for vendor, total in brand_total.items():
        ...
        brand_rows.append(
            {
                "name": vendor,
                "slug": (vendor or "").lower().replace(" ", "-"),
                "source": "bluemercury",
                "url": "https://bluemercury.com/collections/"
                ...
                "give_back": False,
                "cert_coverage": coverage if sustainable else "some",
                "scraped_at": ts,
            }
        )
